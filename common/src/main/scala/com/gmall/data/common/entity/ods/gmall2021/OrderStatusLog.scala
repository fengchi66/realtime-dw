...
class OrderStatusLog extends OdsModel {
...
var operate_time: String = _
  ...
  override def toString = s"OrderStatusLog(database=$database, table=$table, sqlType=$sqlType, ts=$ts, old=$old, id=${id}, order_id=${order_id}, order_status=${order_status}, operate_time=${operate_time})"
