...
实时OLAP应用最多的场景就是数据分析。以下介绍几种数据分析模型，更多的从数据分析、产品的角度去理解需求，不专注于接口的定义以及API实现。
...
- OLAP系统的简介，主要分为MOLAP、ROLAP两种类型。
- 常见的OLAP：Prosto、Druid、ClickHouse、Hologres
- OLAP最重要的领域是数据分析，关于用户行为分析，有几种常见的数据分析模型，这个慢慢学习再总结，内容有点多的。
